// examples\v3\SDK_Examples_file_5_Send_ACME_ADI_to_ADI_v3.dart
//
...
Future<void> main() async {
  print("=== SDK Example 5: ADI-to-ADI Token Transfer ===\n");
...
Future<void> testFeatures() async {
  ...
  try {
    ...
    String adiName = "sdk-transfer-${DateTime.now().millisecondsSinceEpoch}";
    ...
    // Create "tokens" and "staking" accounts
    String tokensAccountUrl = "$identityUrl/tokens";
    String stakingAccountUrl = "$identityUrl/staking";

    print("Creating tokens account: $tokensAccountUrl");
    final createTokensResult = await adiSigner.signSubmitAndWait(
    ...
    if (createTokensResult.success) {
      print("CreateTokenAccount (tokens) SUCCESS - TxID: ${createTokensResult.txid}");
    } else {
      print("CreateTokenAccount (tokens) FAILED: ${createTokensResult.error}");
    }

    print("Creating staking account: $stakingAccountUrl");
    final createStakingResult = await adiSigner.signSubmitAndWait(
      principal: identityUrl,
      body: TxBody.createTokenAccount(
        url: stakingAccountUrl,
        tokenUrl: "acc://ACME",
      ),
    ...
    if (createStakingResult.success) {
      print("CreateTokenAccount (staking) SUCCESS - TxID: ${createStakingResult.txid}\n");
    } else {
      print("CreateTokenAccount (staking) FAILED: ${createStakingResult.error}");
    }
    ...
    final fundAmount = 10 * pow(10, 8).toInt(); // 10 ACME
    print("Sending 10 ACME from lite to $tokensAccountUrl");
    ...
    try {
      final tokensQuery = await client.v3.rawCall("query", {
        "scope": tokensAccountUrl,
        "query": {"queryType": "default"}
      });
      final tokensBalance = tokensQuery["account"]?["balance"];
      print("Tokens account balance: $tokensBalance\n");
    } catch (e) {
      print("Could not query tokens balance: $e\n");
    }
    ...
    final transferAmount = 5 * pow(10, 8).toInt(); // 5 ACME
    print("Sending 5 ACME from $tokensAccountUrl to $stakingAccountUrl");
    print("(ADI-to-ADI transfer using SmartSigner)\n");

    final transferResult = await adiSigner.signSubmitAndWait(
    ...
    if (transferResult.success) {
      ...
      print("TxID: ${transferResult.txid}\n");
    } else {
      print("ADI-to-ADI Transfer FAILED: ${transferResult.error}");
    }
    ...
    try {
      ...
      final stakingQuery = await client.v3.rawCall("query", {
        "scope": stakingAccountUrl,
        "query": {"queryType": "default"}
      });
      final stakingBalance = stakingQuery["account"]?["balance"];
      print("Staking account balance: $stakingBalance\n");
    } catch (e) {
      print("Could not verify balances: $e\n");
    }
    ...
    print("Tokens Account: $tokensAccountUrl");
    print("Staking Account: $stakingAccountUrl");
    print("\nToken transfers:");
