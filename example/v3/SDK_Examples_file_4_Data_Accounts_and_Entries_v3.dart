// examples\v3\SDK_Examples_file_4_Data_Accounts_and_Entries_v3.dart
//
...
Future<void> main() async {
  print("=== SDK Example 4: Data Accounts and Entries ===\n");
...
Future<void> testFeatures() async {
  ...
  try {
    ...
    String adiName = "sdk-data-${DateTime.now().millisecondsSinceEpoch}";
    ...
    String dataAccountUrl = "$identityUrl/data";
    print("Data Account URL: $dataAccountUrl");

    final createDataResult = await adiSigner.signSubmitAndWait(
      principal: identityUrl,
      body: TxBody.createDataAccount(
        url: dataAccountUrl,
      ),
    ...
    // Wait for data account to be created
    ...
    final dataEntry1 = Uint8List.fromList(utf8.encode("Hello, Accumulate!"));
    final dataEntry2 = Uint8List.fromList(utf8.encode("Data entry from Dart SDK"));
    final timestamp = DateTime.now().toIso8601String();
    final dataEntry3 = Uint8List.fromList(utf8.encode("Timestamp: $timestamp"));
    ...
    print("Writing ${entriesHex.length} data entries:");
    print("  1: ${utf8.decode(dataEntry1)}");
    print("  2: ${utf8.decode(dataEntry2)}");
    print("  3: ${utf8.decode(dataEntry3)}\n");

    final writeDataResult = await adiSigner.signSubmitAndWait(
      principal: dataAccountUrl,
      body: TxBody.writeData(
        entriesHex: entriesHex,
        writeToState: false,
      ),
    ...
    try {
      final dataQuery = await client.v3.rawCall("query", {
        "scope": dataAccountUrl,
        "query": {"queryType": "default"}
      });
      print("Data Account Query:");
      print("  URL: ${dataQuery["account"]?["url"]}");
      print("  Type: ${dataQuery["account"]?["type"]}");

      // Query the data chain for entries
      final chainQuery = await client.v3.rawCall("query", {
        "scope": dataAccountUrl,
        "query": {
          "queryType": "chain",
          "name": "main",
      ...
      print("  Main chain entries: ${chainQuery["total"] ?? "unknown"}\n");
    } catch (e) {
      print("Could not query data account: $e\n");
    }
    ...
    print("Created Data Account: $dataAccountUrl");
    print("Wrote ${entriesHex.length} data entries");
